# Real-ESRGAN на спутниковых данных

Этот форк основан на [Real-ESRGAN](https://github.com/xinntao/Real-ESRGAN)

В этом форке изменен загрузчик данных для обучения модели.
Принципиальное отличие в том, что добавлено маштабирование кропов, которые извлекаются из изображения, чтобы симитировать обучение на разных пространственных разрешениях спутниковых данных.

Скрипты запуска обучения находятся в `slurm_scripts`

**Идея обучения заключается в том, чтобы сначала обучить модель на разных вариантах пространственных разрешений. А затем зафайнтьюннить модель на фиксированном разрешении**

Модифицированный класс датасета `RealESRGANDatasetMod` реализован в `realesrgan/data/realesrgan_mod_dataset.py`.
В ноутбуке `test_mod_dataset.ipynb` тестировал класс датасета.

**Конфигурационные файлы** для задачи приведены в директории `options` с суффиксом `_mod`, без суффикса - оригинальные.
Для модифицированного датасета введены два новых параметра:
- `crop_increase_mode` - режим выбора множителя увеличения размера кропа (random, max, fixed)
- `crop_increase` - список чисел с множителями увеличения размера кропа для режимов 'random' и 'max', для режима 'fixed' - словарь именами поддиректорий датасета в качестве ключа ('mapbox', 'mrd', 'xview', 'xview2' в данном случае) и со значениями множителей увеличения размера кропа для соответствующих пространственных разрешений.

Для обучения на разных разрешениях используем разные масштабные параметры. Например, если в конфиге  
```
gt_size: 256  
crop_increase : [1, 2.5, 5, 10]  
crop_increase_mode : 'random'
```  
то датасет будет брать кроп случайным размером из списка [256, 256\*2.5, 256\*5, 256\*10] а затем уже приводить его к размеру 256. Что для изображения я разрешением 0.5 м/пикс будет соответствовать загрублению до разрешения со значением из списка [0.5, 1.25, 2.5, 5]. Если размер изображения не позволяет извлечь из него кроп такого размера, то он будет уменьшен до подходящего размера.  

Для файнтьюна на фиксированном разрешении используем фиксированные масштабные параметры. 
Например если мы хотим обучить модель на разрешении 2.5 м/пикс, в нашем случае датасеты mapbox, mrd имеют разрешение 1 м/пикс, xview - 0.3 м/пикс, xview2 - 0.5 м/пикс, поэтому в конфиге 
```
crop_increase_mode : 'fixed'
crop_increase: 
  mapbox: 2.5
  mrd: 2.5
  xview: 8
  xview2: 4
```
\* xview2 приводится до 2 м/пикс вместо 2.5 поскольку просто размеры патчей оригинального изображения не позволяют из них извлечь кроп размером 2.5 м/пикс.
